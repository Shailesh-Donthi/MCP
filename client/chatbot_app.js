(function () {
    ...
    const RETRY_ICON = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>';
    ...
    function toggleTableView(messageDiv, btn) {
        ...
        // Re-add actions bar with toggle button and retry button
        const actionsBar = document.createElement('div');
        actionsBar.className = 'msg-actions-bar';
        actionsBar.appendChild(createTableToggleButton(messageDiv, state.isTableView));
        const retryQuery = messageDiv.dataset.query;
        if (retryQuery) {
            actionsBar.appendChild(createRetryButton(messageDiv, retryQuery));
        }
        contentDiv.appendChild(actionsBar);
    ...
    // -----------------------------------------------------------------------
    // Feature: Retry button for query responses
    // -----------------------------------------------------------------------

    function createRetryButton(messageDiv, query) {
        const btn = document.createElement('button');
        btn.className = 'msg-action-btn';
        btn.innerHTML = RETRY_ICON + ' Retry';
        btn.onclick = function () { retryQuery(messageDiv, query, this); };
        return btn;
    }

    async function retryQuery(messageDiv, query, btn) {
        if (!query || !messageDiv) return;

        btn.disabled = true;
        btn.innerHTML = RETRY_ICON + ' Retrying\u2026';
        ...
        try {
            ...
            // Replace message content in-place
            const contentDiv = messageDiv.querySelector('.message-content');
            if (contentDiv) {
                const timeText = messageDiv.dataset.timeLabel || '';
                contentDiv.innerHTML = renderWithCollapsible(responseText);

                // Re-attach table toggle if applicable
                const tableHtml = OutputLayer.renderTableFromLast(data);
                if (tableHtml) {
                    messageDataStore.set(messageDiv, {
                        apiResult: data,
                        tableHtml: tableHtml,
                ...
                // Re-build actions bar
                const actionsBar = document.createElement('div');
                actionsBar.className = 'msg-actions-bar';
                if (tableHtml) {
                    actionsBar.appendChild(createTableToggleButton(messageDiv, false));
                }
                actionsBar.appendChild(createRetryButton(messageDiv, query));
                contentDiv.appendChild(actionsBar);
    ...
    function attachRetryButton(messageDiv, query) {
        if (!messageDiv || !query) return;
        messageDiv.dataset.query = query;

        const contentDiv = messageDiv.querySelector('.message-content');
        if (!contentDiv) return;

        // Find existing actions bar or create one
        let actionsBar = contentDiv.querySelector('.msg-actions-bar');
        if (!actionsBar) {
    ...
    async function sendMessage() {
        ...
        try {
            ...
            attachRetryButton(responseMessage, query);
            maybeWarnBackendContextDesync(data);
            if (data.output && data.output.download && data.output.download.content) {
                OutputLayer.triggerDownload(data.output);
            }
