...
def repair_route(
    ...
    place = extract_place_hint(query) or extract_place_hint(merged)
    unit_hint = extract_unit_hint(query) or extract_unit_hint(merged)
    rank_in_query = extract_rank_hint(query)
    rank_name = rank_in_query or extract_rank_hint(merged)
    ...
    if asks_for_people and asks_for_rank_members:
        ...
        if not fixed_args.get("district_name") and place:
            fixed_args["district_name"] = place
    ...
    if tool_name == "get_personnel_distribution" and not fixed_args.get("district_name") and place:
        fixed_args["district_name"] = place
