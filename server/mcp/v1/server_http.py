...
def route_query_to_tool(query: str, hint: Optional[str] = None) -> tuple[str, dict]:
    ...
    def extract_range_location(q: str) -> Optional[str]:
        patterns = [
            r"(?:in|for|of|under)\s+([A-Za-z\s]+?)\s+ranges?\b",
            r"\brange\s+(?:in|for|of)\s+([A-Za-z\s]+?)(?:\?|$)",
            r"\b(?:the\s+)?([A-Za-z]+(?:\s+[A-Za-z]+){0,2})\s+ranges?\b",
        ]
        for pattern in patterns:
            ...
            value = re.sub(r"\b(?:ig|range)\b", "", value, flags=re.IGNORECASE).strip()
            if not value:
                continue
            if value.lower() in {"ap", "andhra pradesh", "state", "statewide"}:
                return None
            return value.title()
        return None
    ...
    # ==========================================================================
    # PRIORITY 5b: Range count queries
    # ==========================================================================
    asks_range_count = bool(
    ...
    if asks_range_count:
        district = extract_district(query) or extract_range_location(query)
        is_state_scope = bool(
            re.search(r"\b(ap|andhra\s+pradesh|state|statewide)\b", query_lower)
        )
        if district and not is_state_scope:
            args["district_name"] = district
        args["unit_type_name"] = "Range"
        # Count intent only needs a sample row; total is returned in pagination.
        args["page_size"] = 1
    ...
    if re.search(r"(?:list|show|get|display|all)\s+(?:the\s+)?(?:all\s+)?units?|units?\s+(?:in|under|of)", query_lower):
        district = extract_district(query) or extract_range_location(query)
        is_range_query = bool(re.search(r"\branges?\b", query_lower))
        if district:
            args["district_name"] = district
        if is_range_query:
            args["unit_type_name"] = "Range"
