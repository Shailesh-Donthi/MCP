...
class ListUnitsInDistrictTool(BaseTool):
    ...
    async def execute(
        ...
        resolved_unit_type_name: Optional[str] = None
        if not unit_type_id and unit_type_name:
            unit_type_doc = await self._resolve_unit_type_by_name(unit_type_name)
            if not unit_type_doc:
                return self.format_error_response(
                    "NOT_FOUND",
                    f"Unit type not found: {unit_type_name}",
                )
            unit_type_id = str(unit_type_doc["_id"])
            resolved_unit_type_name = str(unit_type_doc.get("name") or "").strip() or None
        ...
        if unit_type_id:
            if not ObjectId.is_valid(unit_type_id):
                return self.format_error_response(
                    "VALIDATION_ERROR",
                    f"Invalid unit_type_id: {unit_type_id}",
                )
        ...
        return self.format_success_response(
            ...
            metadata={
                ...
                "unit_type_name": resolved_unit_type_name or unit_type_name,
            },
        )

    async def _resolve_unit_type_by_name(
        self,
        unit_type_name: str,
    ) -> Optional[Dict[str, Any]]:
        raw = str(unit_type_name or "").strip()
        if not raw:
            return None
        normalized = normalize_common_entity_aliases(raw)
        ...
        escaped_name = SafeQueryBuilder.escape_regex(normalized)
        unit_type = await self.db[Collections.UNIT_TYPE].find_one(
            {"name": {"$regex": escaped_name, "$options": "i"}, "isDelete": False}
        )
        if unit_type:
        ...
        if not best:
            return None
        return await self.db[Collections.UNIT_TYPE].find_one(
            {"name": {"$regex": f"^{re.escape(best)}$", "$options": "i"}, "isDelete": False}
        )
