(function () {
    ...
    const CHAT_THREADS_STORAGE_KEY = 'mcpChatThreadsV1';
    const LEGACY_CHAT_SESSION_STORAGE_KEY = 'chatSessionId';
    ...
    function buildDefaultChatThreadsState() {
        const legacySessionId = localStorage.getItem(LEGACY_CHAT_SESSION_STORAGE_KEY) || undefined;
        const firstChat = createChatThread({ sessionId: legacySessionId });
        return {
    ...
    function saveChatThreadsState() {
        if (!chatThreadsState) return;
        try {
            localStorage.setItem(CHAT_THREADS_STORAGE_KEY, JSON.stringify(chatThreadsState));
        } catch (error) {
            console.warn('Failed to persist chat threads', error);
        }
    }

    function loadChatThreadsState() {
        try {
            ...
            const chats = Array.isArray(parsed?.chats)
                ? parsed.chats.map(normalizeChatThread).filter(Boolean)
                : [];
            ...
            console.warn('Failed to load chat threads, resetting state', error);
            chatThreadsState = buildDefaultChatThreadsState();
            saveChatThreadsState();
        }
    }
    ...
    function clearAllChats() {
        if (!window.confirm('Clear all saved chats from this browser? This cannot be undone.')) {
            return;
        }
    ...
    function maybeWarnBackendContextDesync(apiData) {
        ...
        if (localMessageCount >= 4 && backendHistorySize > 0 && backendHistorySize <= 2) {
            ...
            addChatMessage(
                'Note: This chat was restored locally, but the server conversation memory appears to have reset. Follow-up questions may need the full context restated.',
                { role: 'assistant', persist: false }
            );
        }
    }

    function initializeChatThreadsUi() {
        ensureChatThreadsState();
    ...
    async function sendMessage() {
        ...
        addChatMessage(query, { role: 'user' });
        ...
        try {
            ...
            const responseMessage = addChatMessage(responseText, { role: 'assistant' }).element;
